...
const Dashboard = () => {
  ...
  const handleLearningSubmit = (data) => {
    if (editingEntry) {
      ...
      toast.info("��️ Stardust polished! Your learning record has been updated.", { position: 'bottom-right', autoClose: 2500 });
    } else {
      setLearningEntries(entries => [...entries, { ...data, id: Date.now() }]);
      toast.success("🚀 Log recorded! You're fueling your cosmic journey.", { position: 'bottom-right', autoClose: 2500 });
    }
    setIsModalOpen(false);
  };
  ...
  const handleDeleteEntry = (id) => {
    setLearningEntries(entries => entries.filter(entry => entry.id !== id));
    toast.error("☄️ Entry deleted. That star has faded from your path.", { position: 'bottom-right', autoClose: 2500 });
  };
