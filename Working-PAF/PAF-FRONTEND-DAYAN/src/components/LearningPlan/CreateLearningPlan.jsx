...
export function CosmicJourneyForm({ open, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const cleanedForm = {
        ...
        learningMaterials: [] // Initialize empty array for learning materials
      };
      
      // Add the plan data as a string
      formData.append('plan', JSON.stringify(cleanedForm));
      
      if (file) {
        formData.append('learningMaterial', file);
      }
      
      const token = localStorage.getItem('accessToken');
      console.log('Sending data:', cleanedForm); // Debug log
      
      const response = await axios.post('http://localhost:8080/api/v1/learningplan', formData, {
        headers: {
      ...
      if (response.data) {
        setForm({ title: '', description: '', difficultyLevel: '', duration: 4, certificate: '', price: '', isPublic: true });
        setFile(null);
        if (onSuccess) onSuccess();
      }
    } catch (err) {
      console.error('Error creating learning plan:', err);
      const errorMessage = err.response?.data || err.message || 'Failed to create learning plan.';
          ...
          <div className="flex justify-center">
            <div className="w-full max-w-md bg-gradient-to-br from-purple-100 via-pink-100 to-blue-100 rounded-xl shadow-lg p-6 border-2 border-purple-300">
              ...
              <div className="mb-2 flex items-center gap-2">
                ...
                <span className="text-gray-700">File: <span className="font-semibold text-purple-700">{file ? file.name : 'None'}</span></span>
              </div>
              <div className="mb-2 flex items-center gap-2">
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl p-4 sm:p-8 relative border-4 border-purple-300 sm:rounded-2xl rounded-lg max-h-[95vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex justify-between gap-2 mt-6">
            ...
            {step === steps.length - 1 && <button type="submit" disabled={loading} className="px-6 py-2 bg-gradient-to-r from-purple-600 to-pink-500 text-white rounded-lg font-bold shadow hover:from-purple-700 hover:to-pink-600 transition disabled:opacity-60 flex items-center gap-1">{loading ? 'Creating...' : <>Finish <FaCheckCircle /></>}</button>}
          </div>
        </form>
      </div>
    </div>
...
export default CosmicJourneyForm; 